...
public struct AXBridge {
    ...
    /// Find app by PID
    public static func findApp(pid: pid_t) -> NSRunningApplication? {
        NSWorkspace.shared.runningApplications.first { $0.processIdentifier == pid }
    }

    /// Get AXUIElement for an app
    public static func appElement(for app: NSRunningApplication) -> AXUIElement {
        AXUIElementCreateApplication(app.processIdentifier)
    }

    /// List all GUI apps
    public static func listApps() -> [AppInfo] {
    ...
    public static func resolveApp(name: String?, pid: Int32?) -> NSRunningApplication? {
        if let pid = pid {
